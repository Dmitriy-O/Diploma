...
function ResultCard({ method, data, onImageClick, onDownloadClick }) {
    ...
    const processingTime = data.processing_time ? `${data.processing_time.toFixed(2)} сек` : 'N/A';
    ...
    const images = [
        { src: data.upscaled_image_base64, alt: `Увеличенное (${method})`, label: 'Рез.' },
        { src: data.diff_image_base64, alt: `Разница (${method})`, label: 'Разн.' },
        { src: data.hist_base64, alt: `Гистограмма ошибок (${method})`, label: 'Гист.' },
    ];
    ...
    return (
        <div className="text-center result-card bg-gray-700/80 backdrop-blur-sm rounded-xl p-4 shadow-lg border border-gray-600 flex flex-col h-full transition-shadow hover:shadow-indigo-500/20">
                        ...
                        <div key={index} className="relative group">
                            ...
                            >
                                ...
                                <button
                                    ...
                                    title={`Скачать ${image.label}`}
                                    aria-label={`Скачать ${image.label} (${method})`}
                                >
                                    <Download size={14} />
                                </button>
                            </div>
            ...
            <div className="flex justify-center space-x-2 mb-3 flex-wrap gap-2">
                ...
                >
                    <Eye size={14} className="inline mr-1"/> Все
                </button>
            </div>
            <div className="text-sm text-gray-400 space-y-1 mt-auto">
                ...
                <p>
                    <span title="Время обработки метода интерполяции">Время:</span> <span className="font-medium text-gray-200">{processingTime}</span>
                </p>
            </div>
        </div>
    );
